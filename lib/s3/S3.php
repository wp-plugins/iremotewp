...
class S3 {
	...
	*/
	public static function listBuckets($detailed = false) {
		...
		if ($rest->error !== false) {
			return sprintf("S3::listBuckets(): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
		$results = array();
	...
	*/
	public static function getBucket($bucket, $prefix = null, $marker = null, $maxKeys = null, $delimiter = null, $returnCommonPrefixes = false) {
		...
		if ($response->error !== false) {
			return sprintf("S3::getBucket(): [%s] %s", $response->error['code'], $response->error['message']);
		}
	...
	*/
	public static function putBucket($bucket, $acl = self::ACL_PRIVATE, $location = false) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::putBucket({$bucket}, {$acl}, {$location}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
		return true;
	}
	...
	*/
	public static function deleteBucket($bucket) {
		...
		if ($rest->error === false && $rest->code !== 204)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::deleteBucket({$bucket}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
		return true;
	}
	...
	*/
	public static function inputFile($file, $md5sum = true) {
		if (!file_exists($file) || !is_file($file) || !is_readable($file)) {
			return 'S3::inputFile(): Unable to open input file: '.$file;
		}
	...
	*/
	public static function inputResource(&$resource, $bufferSize, $md5sum = '') {
		if (!is_resource($resource) || $bufferSize < 0) {
			return 'S3::inputResource(): Invalid resource or buffer size';
		}
	...
	*/
	public static function putObject($input, $bucket, $uri, $acl = self::ACL_PRIVATE, $metaHeaders = array(), $requestHeaders = array()) {
		...
		if ($rest->response->error !== false) {
			return sprintf("S3::putObject(): [%s] %s", $rest->response->error['code'], $rest->response->error['message']);
		}
		return true;
	}
	...
	*/
	public static function getObject($bucket, $uri, $saveTo = false) {
		...
		if ($rest->response->error === false && $rest->response->code !== 200)
			$rest->response->error = array('code' => $rest->response->code, 'message' => 'Unexpected HTTP status');
		if ($rest->response->error !== false) {
			return sprintf("S3::getObject({$bucket}, {$uri}): [%s] %s", $rest->response->error['code'], $rest->response->error['message']);
		}
	...
	*/
	public static function getObjectInfo($bucket, $uri, $returnInfo = true) {
		...
		if ($rest->error === false && ($rest->code !== 200 && $rest->code !== 404))
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::getObjectInfo({$bucket}, {$uri}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
	...
	*/
	public static function copyObject($srcBucket, $srcUri, $bucket, $uri, $acl = self::ACL_PRIVATE, $metaHeaders = array(), $requestHeaders = array()) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::copyObject({$srcBucket}, {$srcUri}, {$bucket}, {$uri}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
	...
	*/
	public static function setBucketLogging($bucket, $targetBucket, $targetPrefix = null) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::setBucketLogging({$bucket}, {$uri}): [%s] %s", $rest->error['code'], $rest->error['message']);
			return false;
		}
		return true;
	}
	...
	*/
	public static function getBucketLogging($bucket) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::getBucketLogging({$bucket}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
	...
	*/
	public static function getBucketLocation($bucket) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::getBucketLocation({$bucket}): [%s] %s", $rest->error['code'], $rest->error['message']);
		}
	...
	*/
	public static function setAccessControlPolicy($bucket, $uri = '', $acp = array()) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::setAccessControlPolicy({$bucket}, {$uri}): [%s] %s",
	...
	*/
	public static function getAccessControlPolicy($bucket, $uri = '') {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::getAccessControlPolicy({$bucket}, {$uri}): [%s] %s",
			$rest->error['code'], $rest->error['message']);
		}
	...
	*/
	public static function deleteObject($bucket, $uri) {
		...
		if ($rest->error === false && $rest->code !== 204)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::deleteObject(): [%s] %s",
			$rest->error['code'], $rest->error['message']);
		}
		return true;
	}
	...
	*/
	public static function createDistribution($bucket, $enabled = true, $cnames = array(), $comment = '') {
		...
		if ($rest->error === false && $rest->code !== 201)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::createDistribution({$bucket}, ".(int)$enabled.", '$comment'): [%s] %s",
			$rest->error['code'], $rest->error['message']);
	...
	*/
	public static function getDistribution($distributionId) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::getDistribution($distributionId): [%s] %s",
			$rest->error['code'], $rest->error['message']);
		} elseif ($rest->body instanceof SimpleXMLElement) {
	...
	*/
	public static function updateDistribution($dist) {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::updateDistribution({$dist['id']}, ".(int)$enabled.", '$comment'): [%s] %s",
			$rest->error['code'], $rest->error['message']);
		} else {
			$dist = self::__parseCloudFrontDistributionConfig($rest->body);
			$dist['hash'] = $rest->headers['hash'];
	...
	*/
	public static function deleteDistribution($dist) {
		...
		if ($rest->error === false && $rest->code !== 204)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::deleteDistribution({$dist['id']}): [%s] %s",
			$rest->error['code'], $rest->error['message']);
		}
		return true;
	}
	...
	*/
	public static function listDistributions() {
		...
		if ($rest->error === false && $rest->code !== 200)
			$rest->error = array('code' => $rest->code, 'message' => 'Unexpected HTTP status');
		if ($rest->error !== false) {
			return sprintf("S3::listDistributions(): [%s] %s",
			$rest->error['code'], $rest->error['message']);
...
final class S3Request {
	...
	*/
	...
	private function __progressCallback(&$download_size, &$downloaded, &$upload_size, &$uploaded){

		if($_SESSION['whenS3']['time'] < time()){

        	$_SESSION['whenS3']['time'] 		= time() + 15;
        	$_SESSION['whenS3']['filename']		= $this->uri;
        	$_SESSION['whenS3']['upload_id']	= $this->headers['Date'];
        	$_SESSION['whenS3']['upload_id']	= $this->headers['Date'];
        	$_SESSION['whenS3']['filetotal']	= $upload_size;
        	$_SESSION['whenS3']['sk']			= $_POST['sk'];
					...
					wp_remote_post( IREM_API_URL . 'dropbox/upload.php', array(
						...
						'blocking' => true,
						'headers' => array(),
						'body' => array( 'sk'=> get_option( 'irem_verify_key' ),'filename' => $_SESSION['whenS3']['filename'], 'upload_id' => $_SESSION['whenS3']['upload_id'] , 'offset' => $uploaded, 'filetotal' => $_SESSION['whenS3']['filetotal'], 'parameter' => $_SESSION['whenS3']['sk'] ),
						'cookies' => array()
					    )
					);

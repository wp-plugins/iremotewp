...
class IREMOTE_HM_Backup {
	...
	public function set_archive_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			throw new Exception( __( 'archive filename must be a non empty string', 'iremotewp' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'zip' )
			throw new Exception( __( 'invalid file extension for archive filename', 'iremotewp' ) .  '<code>' . $filename . '</code>' );
	...
	public function set_database_dump_filename( $filename ) {
		...
		if ( empty( $filename ) || ! is_string( $filename ) )
			throw new Exception( __( 'database dump filename must be a non empty string', 'iremotewp' ) );

		if ( pathinfo( $filename, PATHINFO_EXTENSION ) !== 'sql' )
			throw new Exception( __( 'invalid file extension for database dump filename', 'iremotewp' ) . '<code>' . $filename . '</code>' );
	...
	public function set_root( $path ) {
    	...
    	if ( empty( $path ) || ! is_string( $path ) || ! is_dir ( $path ) )
    		throw new Exception( sprintf( __( 'Invalid root path %s must be a valid directory path', 'iremotewp' ), '<code>' . $path . '</code>' ) );
	...
	public function set_path( $path ) {
    	...
    	if ( empty( $path ) || ! is_string( $path ) )
    		throw new Exception( sptrinf( __( 'Invalid backup path %s must be a non empty (string)', iremotewp ), '<code>' . $path . '</code>' ) );
	...
	public function set_type( $type ) {
		...
		if ( ! is_string( $type ) || ! in_array( $type, array( 'file', 'database', 'complete' ) ) )
			throw new Exception( sprintf( __( 'Invalid backup type %s must be one of (string) file, database or complete', 'iremotewp' ), '<code>' . $type . '</code>' ) );
	...
	public function pcl_zip() {
		...
		global $_iremo_hmbkp_exclude_string;

		$_iremo_hmbkp_exclude_string = $this->exclude_string( 'regex' );

		$archive = &$this->setup_pclzip();

		// Zip up everything
		if ( $this->get_type() !== 'database' )
			if ( ! $archive->add( $this->get_root(), PCLZIP_OPT_REMOVE_PATH, $this->get_root(), PCLZIP_CB_PRE_ADD, 'iremo_hmbkp_pclzip_callback' ) )
		...
		unset( $GLOBALS['_iremo_hmbkp_exclude_string'] );
 ...
 * @return bool
 */
function iremo_hmbkp_pclzip_callback( $event, &$file ) {
    ...
    elseif ( $_iremo_hmbkp_exclude_string && preg_match( '(' . $_iremo_hmbkp_exclude_string . ')', $file['stored_filename'] ) )
    	return false;
